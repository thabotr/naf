...
export const verboseTime = (timestamp?: number): string | undefined => {
  ...
  const timeDiffSec = Math.floor(
    Math.abs(now.getTime() - time.getTime()) / 1000,
  );
  const min = 60;
  const hour = min * 60;
  const day = hour * 24;
...
export function verboseDuration(seconds: number): string {
  ...
  const minSecs = `${zeroPad(Math.floor(seconds / 60) % 60)}:${zeroPad(
    seconds % 60,
  )}`;
  return seconds / 3600 < 1
    ? minSecs
    : `${zeroPad(Math.floor(seconds / 3600))}:${minSecs}`;
}
