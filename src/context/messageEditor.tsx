...
const MessageComposerContext =
  React.createContext<MessageComposerContextType | null>(null);
...
const MessageComposerProvider = ({children}: Props) => {
  ...
  const [vrState, setVRState] = React.useState<VRState>({
    recordingPermitted: false,
  });
  const [showTextInput, setShowTextInput] = React.useState(false);
  const [recordSecs, setRecordSecs] = React.useState(0);
  const [audioRecorderPlayer] = React.useState<AudioRecorderPlayer | null>(
    null,
  );
  ...
  const onAddAttachments = () => {
    DocumentPicker.pick({allowMultiSelection: true, copyTo: 'cachesDirectory'})
      .then((dprs: Array<DocumentPickerResponse> | null | undefined) => {
        if (dprs) {
