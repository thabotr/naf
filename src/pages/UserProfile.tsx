...
const WaitingForYouList=()=>{
  ...
  <View>
    <HorizontalView>
      <IconButton icon='delete' style={styles.squareButton} onPress={()=>{}}/>
      <List.Accordion
        style={{width: 430, borderWidth: 1, padding: 0}}
        title={'Paris | Johannesburg | Doha'}
      >
        <List.Item
          left={_=><CardCover source={user?.avatarURI} style={{width: 50, height: 50}}/>}
          style={{width: '100%'}}
          title={user?.handle}
          description={`${user?.name} ${user?.surname} [${user?.initials}]`}
          right={_=><>
          <IconButton icon='cancel' style={styles.squareButton} onPress={()=>{}}/>
          <IconButton icon='account-plus' style={styles.squareButton} onPress={()=>{}}/>
          </>}
        />
      </List.Accordion>
    </HorizontalView>
    <Pressable>
      <HorizontalView style={{alignItems: 'center', justifyContent: 'center', borderWidth: 1, paddingHorizontal: 5}}>
        <Show
          component={<>
          ...
          ElseShow={<>
            ...
            <TextInput label={'first place'} value="paris" style={styles.placeText}/>
            <TextInput label={'second place'} value="johannesburg" style={styles.placeText}/>
            <TextInput label={'third place'} value="doha" style={styles.placeText}/>
            </HorizontalView>
            <IconButton icon='content-save' style={styles.squareButton} onPress={()=>{}}/>
          </>}
        />
      </HorizontalView>
    </Pressable>
...
const WaitingForThemList=()=>{
  ...
  >
    <List.Item
      ...
      left={_=><IconButton icon='delete' style={[styles.squareButton,styles.minimalButton, {alignSelf: 'center'}]} onPress={()=>{}}/>}
    />
    <HorizontalView style={{alignItems: 'center', justifyContent: 'center', borderWidth: 1, paddingHorizontal: 5}}>
    <Show
      component={<>
      ...
      <>
        ...
        <TextInput label={'first place'} value="paris" style={{width: 150, marginHorizontal: 1}}/>
        <TextInput label={'second place'} value="johannesburg" style={{width: 150, marginHorizontal: 1}}/>
        <TextInput label={'third place'} value="doha" style={{width: 150, marginHorizontal: 1}}/>
        </ScrollView>
        </HorizontalView>
        <IconButton icon='content-save' style={styles.squareButton} onPress={()=>{}}/>
      </>
...
const ProfilePreview=()=>{
  ...
  return <>
    <View>
    <CardCover source={user?.landscapeURI} style={{width: '100%', opacity: editing ? 0.6 : 1}}/>
    <OnlyShow If={editing}>
      <OverlayedView>
        <HorizontalView>
          <IconButton
            ...
            onPress={()=>{}}
            style={[{backgroundColor: theme.color.primary}, styles.squareButton, styles.minimalButton]}
          />
          <IconButton
            icon='camera'
            onPress={()=>{}}
    ...
    <HorizontalView style={{width: '100%', alignItems: 'center', padding: 10, backgroundColor: theme.color.secondary}}>
      <View>
      <CardCover source={user?.avatarURI} style={{width: 120, height: 120, opacity: editing ? 0.6 : 1}}/>
      <OnlyShow If={editing}>
        <OverlayedView>
          <HorizontalView>
            <IconButton
    ...
    <HorizontalView style={{alignItems: 'center', justifyContent: 'center'}}>
      ...
      <OverlayedView>
        <Button style={{width: '100%'}} onPress={()=>setEditing(editing=>!editing)}> </Button>
      </OverlayedView>
    </HorizontalView>
  </>
}
